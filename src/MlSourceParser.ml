...
(**
  ...
  [globals] <-  <epsilon>
    ...
    [increment_for] <- [set_instr]
             | <epsilon>

    [instr] <-  <epsilon>
             |  print ( [expr] )
             |  [set_instr]
             |  while ( [expr] ) [loopblock]
             |  for ([init_for];[condition_for];[increment_for]) [loopblock]
...
(* [parse_loopblock: token_buffer -> instruction] *)
(* [loop_block] <- BEGIN END 
            | BEGIN [inloopinstr] END *)
and parse_loopblock b =
  expect_token BEGIN b;
  if next_token b = END
  then begin shift b; mk_loc_i Nop b end
  else begin
    let i = parse_inloopinstr b in
    expect_token END b;
    i
  end
...
and parse_s_instr b =
  match next_token b with
    ...
    | WHILE -> shift b; expect_token LP b; let e = parse_expr b in
                                           expect_token RP b;
                                           let i = parse_loopblock b in
                                           mk_loc_i (Loop(e, i, (mk_loc_i Nop b))) b
    | FOR -> shift b; expect_token LP b; let init = parse_init_for b in
					 expect_token SEMI b;
					 let condition = parse_condition_for b in
					 expect_token SEMI b;
					 let increment = parse_increment_for b in
					 expect_token RP b;
					 let i = parse_loopblock b in
					 if (init.instr) = Nop then
					   begin
					     if (increment.instr) = Nop then mk_loc_i (Loop(condition, i, (mk_loc_i Nop b))) b
					     else mk_loc_i (Loop(condition, i, increment)) b
					   end
					 else
					   begin
					     if (increment.instr) = Nop then mk_loc_i (Sequence(init, (mk_loc_i (Loop(condition, i, (mk_loc_i Nop b))) b))) b
					     else mk_loc_i (Sequence(init, (mk_loc_i (Loop(condition, i, increment)) b))) b
					   end
...
and parse_init_for b =
  if (next_token b) = SEMI then mk_loc_i (Nop) b
  else
    begin
      (*shift b;*)
      parse_set_instr b
...
and parse_condition_for b =
  if (next_token b) = SEMI then mk_loc_e (Literal(Bool(true))) b
  else
    begin
      (*shift b;*)
      parse_expr b
...
and parse_increment_for b =
  if (next_token b) = RP then mk_loc_i (Nop) b
  else
    begin
      (*shift b;*)
      parse_set_instr b
...
and parse_set_instr b =
  match next_token b with
    | IDENT id -> shift b; expect_token SET b; let e  = parse_expr b in
                                               mk_loc_i (Set(Identifier(Id id), e)) b
    | t -> failwith (Printf.sprintf "Bad instruction on token %s" (token_to_string t))

and parse_inloopinstr b =
  let i1 = parse_s_loop_instr b in
  match next_token b with
  | SEMI -> shift b; let i2 = parse_inloopinstr b in mk_loc_i (Sequence(i1, i2)) b
  | _    -> i1
     
and parse_s_loop_instr b =
  match next_token b with
  ...
  | IF -> shift b; expect_token LP b; let e = parse_expr b in
                                      expect_token RP b;
                                      let i1 = parse_loopblock b in
                                      expect_token ELSE b;
                                      let i2 = parse_loopblock b in
                                      mk_loc_i (Conditional(e, i1, i2)) b
  | _ -> parse_set_instr b
